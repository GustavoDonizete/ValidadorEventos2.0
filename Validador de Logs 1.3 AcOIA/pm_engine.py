...
def export_to_excel(result: dict, excel_path: str = "pm_engine_log.xlsx") -> str:
    ...
    # ── ABA execucoes ──
    EXEC_HEADERS = [
        "id_execucao", "anomes", "nome_base", "aceleradora_oia", "responsavel", "data_execucao", "ts_inicio", "ts_fim",
        "tempo_execucao",
    ...
    col_oia       = EXEC_HEADERS.index("aceleradora_oia") + 1
    ...
    # Aceleradora OIA
    oia_cell = ws_exec.cell(new_row_idx, col_oia)
    if oia_cell.value == "Sim":
        oia_cell.fill = PatternFill("solid", fgColor="FFF3CD")   # amarelo suave
        oia_cell.font = Font(name="Arial", size=10, color="7D5800", bold=True)
    else:
        oia_cell.fill = PatternFill("solid", fgColor=WHITE)
        oia_cell.font = Font(name="Arial", size=10, color="666666")
    ...
    def analyze():
        try:
            ...
            try:
                ...
                # Flag Aceleradora OIA — vem do frontend, gravada no log Excel
                result["aceleradora_oia"] = bool(config.get("aceleradora_oia", False))
