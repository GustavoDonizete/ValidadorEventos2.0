...
def score_pillar1(df: pd.DataFrame, case_col: str, act_col: str,
    ...
    nr_end_invalid = 0.0
    if end_col:
        end_parsed = pd.to_datetime(df[end_col], errors="coerce", dayfirst=dayfirst)
    ...
    start_parsed = pd.to_datetime(df[start_col], errors="coerce", dayfirst=dayfirst)
...
def score_pillar2(df: pd.DataFrame, case_col: str, act_col: str,
    ...
    max_events    = float(events_per_case.max())    if total_cases > 0 else 0

    # Threshold da mediana: mediana ideal = mediana * max (mínimo 1 para evitar /0)
    # O motor calcula a mediana de referência como a mediana dos eventos por case
    # multiplicada pelo máximo — capturando a escala real da base.
    median_ref = float(np.median(events_per_case) * max_events) if total_cases > 0 else 0

    def pts_pct_lt2(rate):
        if rate <= 0.50:    return 35
        if rate <= 0.65:    return 26
        if rate <= 0.80:    return 14
        if rate <= 0.90:    return 6
        return 0

    def pts_median(m, ref):
        """Pontua a mediana relativa ao threshold calculado (mediana * max)."""
        if ref <= 0:
            return 15 if m >= 2 else 0
        if m >= ref:    return 15
        if m >= ref * 0.66: return 12
        if m >= ref * 0.50: return 8
        if m >= 2:      return 4
        return 0

    s22 = pts_pct_lt2(pct_lt2) + pts_median(median_events, median_ref)
    ...
    # ── Gates ──
    if pct_lt2 > 0.95:
        gates_triggered.append(f"Cases com < 2 eventos > 95% ({pct_lt2*100:.1f}%)")
    if median_events < 2:
    ...
    if pct_lt2 > 0.50:
        diagnostics.append({
            ...
            "threshold": "<=50%",
            "severidade": "CRITICO" if pct_lt2 > 0.95 else "ALERTA",
    ...
    if median_events < median_ref:
        diagnostics.append({
            "check": "Mediana baixa de eventos por case",
            "valor": f"{median_events:.1f} eventos",
            "threshold": f">={median_ref:.1f} (mediana × max calculado)",
            "severidade": "CRITICO" if median_events < 2 else "ALERTA",
            "impacto": f"-{15 - pts_median(median_events, median_ref)} pts",
            "descricao": f"A mediana de eventos por case e {median_events:.1f}. O threshold calculado para esta base e {median_ref:.1f} (mediana × maximo). Para Process Mining significativo, o ideal e atingir ou superar esse valor."
        })
    if period_days < 30:
        diagnostics.append({
    ...
    return {
        ...
        "metrics": {
            ...
            "max_events_per_case":    round(max_events, 0),
            "median_ref_threshold":   round(median_ref, 1),
...
def score_pillar3(df: pd.DataFrame, case_col: str, act_col: str,
    ...
    # Trabalhar apenas com linhas válidas
    valid = df[[case_col, act_col, start_col]].copy()
    if end_col:
        valid = df[[case_col, act_col, start_col, end_col]].copy()
    valid = valid[valid[case_col].notna() & valid[act_col].notna()]
    ...
    start_parsed_p3 = pd.to_datetime(df[start_col], errors="coerce", dayfirst=dayfirst)
    active_dates_p3 = start_parsed_p3.dropna().dt.date
...
def score_pillar6(df: pd.DataFrame, case_col: str, act_col: str,
    ...
    sorted_valid = valid[valid["__start"].notna()].sort_values([case_col, "__start"])
...
def run_analysis(input_path: str, case_col: str, act_col: str,
    ...
    # Combina start e end para ter amostra maior
    ...
    # Se dayfirst foi passado explicitamente, respeita; senao usa o inferido
    if dayfirst is None:
    ...
    try:
        ts_parsed  = pd.to_datetime(df[start_col], errors="coerce",
                                    dayfirst=dayfirst_used)
        periodo_dias = int((ts_parsed.max() - ts_parsed.min()).days)
    except Exception:
        periodo_dias = 0
...
def export_to_excel(result: dict, excel_path: str = "pm_engine_log.xlsx") -> str:
    ...
    ws_exec.append(exec_row_data)
    new_row_idx = ws_exec.max_row
    alt = (new_row_idx % 2 == 0)
    ...
    for i, drow in enumerate(rows_diag):
        ...
        ws_diag.append(diag_row_data)
        diag_idx = ws_diag.max_row
        alt_d = (diag_idx % 2 == 0)
    ...
    for srow in rows_sub:
        ...
        ws_sub.append(sub_row_data)
        sub_idx = ws_sub.max_row
        alt_s   = (sub_idx % 2 == 0)
...
def start_server(port: int = 5000):
    ...
    def health():
        return jsonify({"status": "ok", "engine": "pm_engine v1.0"})
